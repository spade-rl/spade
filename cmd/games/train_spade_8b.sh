...
OUTPUT_DIR="${OUTPUT_DIR:-/scratch/spade_games_8b/$(date +%Y%m%d_%H%M%S)}"
...
source "${PROJECT_ROOT}/cmd/models/qwen3-8B.sh"
...
case "${REWARD_VARIANT}" in
   blend) SAVE_NAME="Qwen3-8B_games_blend_thinking" ;;
   regret_var) SAVE_NAME="Qwen3-8B_games_regret_var_thinking" ;;
   *) echo "ERROR: REWARD_VARIANT must be 'blend' or 'regret_var', got '${REWARD_VARIANT}'" >&2; exit 1;;
esac
echo "[A/B] Proposer reward arm: ${REWARD_VARIANT}"

CKPT_ARGS=(
   --hf-checkpoint "${MODEL_ROOT}/Qwen3-8B"
   --ref-load "${MODEL_ROOT}/Qwen3-8B_torch_dist"
   ${LOAD_DIR:+--load "${LOAD_DIR}"}
   --save "${OUTPUT_DIR}/${SAVE_NAME}/"
   --save-interval 16
)
...
if [ "${ACTOR_THINKING:-1}" = "1" ]; then
   echo "[ACTOR-THINKING] Enabled — per-turn thinking samples for the actor"
...
if [ "${NO_PROPOSER_TRAIN:-0}" = "1" ]; then
   echo "[NO-PROPOSER] Proposer training disabled"
...
WANDB_ARGS=(
   ...
   --wandb-group "${WANDB_GROUP:-spade-games-8b}"
...
RUNTIME_ENV_JSON="{
  \"env_vars\": {
    ...
    \"LITELLM_LOG\": \"WARNING\",
    \"SPARE_ROLLOUT_RLIMIT_GB\": \"${SPARE_ROLLOUT_RLIMIT_GB:-400}\"
  }
}"

RAY_JOB_ID="spade_games_8b_$$"
