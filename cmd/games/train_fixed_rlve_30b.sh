...
OUTPUT_DIR="${OUTPUT_DIR:-/scratch/fixed_rlve_30b_${RLVE_ENV_SET}/$(date +%Y%m%d_%H%M%S)}"
...
CKPT_ARGS=(
   ...
   --save "${OUTPUT_DIR}/Qwen3-30B-A3B-Instruct-2507_fixed_rlve_${RLVE_ENV_SET}/"
   --save-interval 16
)
...
WANDB_ARGS=(
   ...
   --wandb-group "${WANDB_GROUP:-fixed-rlve-30b-${RLVE_ENV_SET}}"
...
RAY_JOB_ID="fixed_rlve_30b_${RLVE_ENV_SET}_$$"
