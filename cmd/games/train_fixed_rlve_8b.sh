...
OUTPUT_DIR="${OUTPUT_DIR:-/scratch/fixed_rlve_8b_${RLVE_ENV_SET}/$(date +%Y%m%d_%H%M%S)}"
...
CKPT_ARGS=(
   ...
   --save "${OUTPUT_DIR}/Qwen3-8B_fixed_rlve_${RLVE_ENV_SET}/"
   --save-interval 16
)
...
WANDB_ARGS=(
   ...
   --wandb-group "${WANDB_GROUP:-fixed-rlve-8b-${RLVE_ENV_SET}}"
...
RAY_JOB_ID="fixed_rlve_8b_${RLVE_ENV_SET}_$$"
