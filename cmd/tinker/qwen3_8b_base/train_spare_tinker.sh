...
OUTPUT_DIR="${OUTPUT_DIR:-./experiments/spare_tinker/qwen3_8b_base_$(date +%Y%m%d_%H%M%S)}"
...
ENV_MAX_TOKENS="${ENV_MAX_TOKENS:-13000}"
...
GAMMA="${GAMMA:-0.99}"
USE_SOLVER_VARIANCE_REWARD="${USE_SOLVER_VARIANCE_REWARD:-false}"
...
python3 train_spare_tinker.py \
    ...
    wandb_project="spare" \
    wandb_name="spare_tinker_qwen3_8b_base_$(date +%Y%m%d_%H%M%S)" \
    use_weave=true \
