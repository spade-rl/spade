...
OUTPUT_DIR="${OUTPUT_DIR:-./experiments/spare_tinker/qwen3_30b_a3b_instruct_$(date +%Y%m%d_%H%M%S)}"
...
python3 train_spare_tinker.py \
    ...
    wandb_project="spare" \
    wandb_name="spare_tinker_qwen3_30b_a3b_instruct_$(date +%Y%m%d_%H%M%S)" \
    use_weave=true \
