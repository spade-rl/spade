...
RENDERER_NAME="${RENDERER_NAME:-gpt_oss_medium_reasoning}"
OUTPUT_DIR="${OUTPUT_DIR:-./experiments/spare_tinker/gpt_oss_20b_$(date +%Y%m%d_%H%M%S)}"
...
python3 train_spare_tinker.py \
    ...
    wandb_project="spare" \
    wandb_name="spare_tinker_gpt_oss_20b_$(date +%Y%m%d_%H%M%S)" \
    use_weave=true \
